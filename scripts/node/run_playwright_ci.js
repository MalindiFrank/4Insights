...
async function main() {
  try {
    ...
    console.log(
      "Frontend ready. Waiting for collector metrics at http://localhost:8000/4insights/metrics ...",
    );
    ...
    const play = spawn("npx", ["playwright", "test", "--project=chromium"], {
    ...
    if (code !== 0) {
      throw new Error("Playwright tests failed with code " + code);
    }
